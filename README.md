...
```
* **measured_B0_offset**  ：计算前后匀场差别的算法，将以下路径换成后缀为'_regis_deltB0_ppm'的即可，在此例子中，sham作为reference，shim作为匀场后的数据
```
...
```
## Spherical Harmonics:
在SH.ipynb中添加了球谐二阶的分解过程，其实就是调包（，输出各分量的系数并用直方图表示
