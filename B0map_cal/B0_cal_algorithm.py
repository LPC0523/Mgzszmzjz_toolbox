    ...
    for directory in directories:
        try:
            ...
            if phase_map and magnitude_map:
                ...
                fslmaths(unwrap_path).mul(mask_path).run(brain_map_path)



                # Calculate delta B0 in Hz
                deltB0_hertz_path = os.path.join(nifti_file_path, f"{directory}_deltaB0_hertz.nii.gz")
                fslmaths(brain_map_path).div(2).div(np.pi).div(delta_TE).div(1e6).run(deltB0_hertz_path)
...
if __name__ == "__main__":
    dicom_directory = r'B0map_cal/Dicom/SF_map_processing'
    nifti_folder = r'B0map_cal/Dicom/SF_map_processing_nii'
    delta_TE = 0.00246
    Frequency = 42.58 * 5
