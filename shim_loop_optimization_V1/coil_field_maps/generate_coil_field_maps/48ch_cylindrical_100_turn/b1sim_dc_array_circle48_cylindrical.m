% Calculates B0 field components generated by 48 channel gapped cylindrical
% array of 100-turn shim loops described in:
%
% Juchem C, et al., "Dynamic multi-coil shimming of the human brain at 7 T",
% J. Magn. Reson., 2011.  
%
%
...
output_stem='dc_array_circle48_cylindrical';


% recalculate coordinates of loops for reference
da = 230;   db = 200;  
y1 = da/2*cos([0:1:11]*2*pi/12);
phi1 = 2*pi/24;
% x1 = da*sqrt(1-y1.^2./(db/2.^2));
x1 = db/2*sin([0:1:11]*2*pi/12);

y2 = da/2*cos([0:1:11]*2*pi/12 + phi1);
x2 = db/2*sin([0:1:11]*2*pi/12 + phi1);

zz = [-123.5 -76.5  76.5  123.5];

aa=0;  bb=0;  cc=0;  dd=0; 
for ii = 1:48, ii
    if ii >= 1 && ii <= 12
        aa=aa+1;
        out_text(2*ii-1,:) = [x1(aa) y1(aa) zz(ceil(ii/12))];
    end
    if ii >= 13 && ii <= 24
        bb=bb+1;
        out_text(2*ii-1,:) = [x2(bb) y2(bb) zz(ceil(ii/12))];
    end
    if ii >= 25 && ii <= 36
        cc=cc+1;
        out_text(2*ii-1,:) = [x1(cc) y1(cc) zz(ceil(ii/12))];
    end
    if ii >= 37 && ii <= 48
        dd=dd+1;
        out_text(2*ii-1,:) = [x2(dd) y2(dd) zz(ceil(ii/12))];
    end
end
for ii=1:48
    out_text(2*ii,:) = [1.01*out_text(2*ii-1,1) 1.01*out_text(2*ii-1,2) out_text(2*ii-1,3)];
end
% this geometry in 'out_text' should match the text file loaded below....




[cx,cy,cz]=textread('array_circle48_cylindrical.txt','%f %f %f');

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
circle_radius=47/2; % radius hard-coded for 48ch cylindrical array
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% adjust coordinate origin geometry slightly
cz=cz+16;
cy=cy+5;

ports=48; % number of channels 
...
%%%%%%%%%%% save data %%%%%%%%%%%%%%
 save b1_48ch_cylindrical_save.mat b1_z fov_x fov_y fov_z
